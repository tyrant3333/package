...
app.use((req, res, next) => {
  ...
  if (!uniqueIPs.has(ip)) { uniqueIPs.add(ip); saveVisitors(); }
  const liveKey = ip + ua;
  const now = Date.now();
  if (activeUsers.has(liveKey)) {
    activeUsers.get(liveKey).lastSeen = now;
  } else {
...
setInterval(() => {
  ...
  for (let [key, val] of activeUsers) {
    if (now - val.lastSeen > 15000) activeUsers.delete(key);
  }
}, 10000);
...
app.post('/api/videos/:id/view', (req, res) => {
  try {
    ...
    if (video) { video.views = (video.views || 0) + 1; writeData(videos); }
    const ip = req.ip || '0.0.0.0'; const ua = req.headers['user-agent'] || '';
    const liveKey = ip + ua;
    if (activeUsers.has(liveKey)) activeUsers.get(liveKey).currentVideo = id;
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: 'Error' }); }
});
app.get('/api/config', (req, res) => {
  try { res.json(readConfig()); } catch (e) { res.status(500).json({ error: 'Config error' }); }
});

// ==========================================
// 🔐 ADMIN LOGIN (MUST BE BEFORE MIDDLEWARE)
// ==========================================
...
// ==========================================
// 🛡️ ADMIN AUTH MIDDLEWARE (FOR ALL /admin PATHS EXCEPT LOGIN)
// ==========================================
      ...
      * { margin:0; padding:0; box-sizing:border-box; }
      ...
      <div class="admin-container">
        ...
        <div class="section" style="border-color: rgba(124,92,252,0.3);">
          ...
          <form id="telegramForm">
            <div class="form-group"><label>Telegram Channel Link</label><input type="text" id="telegramLink" placeholder="https://t.me/your_channel" /></div>
            <div class="form-group"><label style="display:flex;"><input type="checkbox" id="notificationEnabled" /> Enable Notification Banner</label></div>
            <div class="form-group"><label style="display:flex;"><input type="checkbox" id="permanentNotification" /> Permanent Banner</label></div>
            <div style="border-top:1px solid rgba(255,255,255,0.05); padding-top:16px;">
              <div class="form-group"><label>Bot Token</label><input type="text" id="botToken" placeholder="123456:ABC-DEF1234ghIkl..." /></div>
              <div class="form-group"><label>Channel ID</label><input type="text" id="channelId" placeholder="@your_channel" /></div>
            </div>
      ...
      <script>
        ...
        async function fetchStats(){
          try { const res=await fetch('/admin/api/stats'); const data=await res.json();
            document.getElementById('totalUsers').textContent=data.totalUsers; document.getElementById('liveUsers').textContent=data.liveUsers;
            document.getElementById('totalVideos').textContent=data.totalVideos; document.getElementById('totalViews').textContent=data.totalViews;
            const lb=document.getElementById('liveBody'); lb.innerHTML='';
            if(data.liveVideoStats && data.liveVideoStats.length){ data.liveVideoStats.forEach(item=>{ lb.innerHTML+='<tr><td>'+item.title+'</td><td>'+item.count+'</td></tr>'; }); }
            else { lb.innerHTML='<tr><td colspan="2">No one watching right now</td></tr>'; }
          }catch(e){}
        }
        async function loadVideos(){ const res=await fetch('/api/videos'); const videos=await res.json();
          const body=document.getElementById('videoBody'); body.innerHTML='';
          videos.forEach(v=>{ body.innerHTML+='<tr><td>'+v.title+'</td><td>'+(v.views||0)+'</td><td>'+new Date(v.uploadedAt).toLocaleDateString()+'</td><td><button class="del-btn" onclick="deleteVideo(\\''+v.id+'\\')"><i class="fas fa-trash"></i></button></td></tr>'; });
        }
        ...
        async function loadTelegramConfig() {
          try { const res=await fetch('/admin/api/config'); const config=await res.json();
        ...
        document.getElementById('telegramForm').addEventListener('submit', async function(e) {
          e.preventDefault(); const btn=document.getElementById('telegramSaveBtn');
          btn.disabled=true; btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Saving...';
          const data={ telegramLink: document.getElementById('telegramLink').value.trim(), notificationEnabled: document.getElementById('notificationEnabled').checked, permanentNotification: document.getElementById('permanentNotification').checked, botToken: document.getElementById('botToken').value.trim(), channelId: document.getElementById('channelId').value.trim() };
          try { const res=await fetch('/admin/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
        ...
        document.getElementById('uploadForm').addEventListener('submit',async(e)=>{
          e.preventDefault(); const title=document.getElementById('title').value.trim(); const caption=document.getElementById('caption').value.trim(); const file=document.getElementById('videoFile').files[0];
          if(!file||!title){ showToast('Fill all fields','error'); return; }
          const btn=document.getElementById('submitBtn'); const prog=document.getElementById('uploadProgress'); const bar=document.getElementById('progressBar');
          ...
          try { const res=await fetch('/admin/api/upload',{method:'POST',body:formData}); clearInterval(interval);
        ...
        setInterval(fetchStats,5000); loadVideos(); fetchStats(); loadTelegramConfig();
      </script>
    </body></html>
  `);
});
  ...
  try {
    ...
    if (config.botToken && config.channelId) {
      try {
        ...
        const tgRes = await axios.post(`https://api.telegram.org/bot${config.botToken}/sendVideo`, form, { headers: form.getHeaders() });
        if (tgRes.data.ok) {
          ...
          if (fileRes.data.ok) telegramUrl = `https://api.telegram.org/file/bot${config.botToken}/${fileRes.data.result.file_path}`;
        }
      } catch (teleErr) { console.warn('⚠️ Telegram upload failed'); }
    }
    const newVideo = { id: Date.now().toString(), title, caption, filename: req.file.originalname, fileUrl: '/uploads/' + folderName + '/' + req.file.originalname, telegramUrl, uploadedAt: new Date().toISOString(), views: 0 };
    videos.push(newVideo); writeData(videos);
    res.json(newVideo);
...
app.delete('/admin/api/videos/:id', (req, res) => {
  try {
    ...
    videos.splice(index, 1); writeData(videos);
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: 'Delete failed' }); }
});

app.get('/admin/api/config', (req, res) => { try { res.json(readConfig()); } catch (e) { res.status(500).json({ error: 'Config error' }); } });
app.post('/admin/api/config', (req, res) => {
  try {
    ...
    config.botToken = botToken || ''; config.channelId = channelId || '';
    writeConfig(config); res.json(config);
...
// ==========================================
// 🛑 CATCH-ALL (SAFE - NO REDIRECT LOOP)
// ==========================================
app.get('*', (req, res) => {
  if (req.path.startsWith('/admin')) {
    // Agar koi bhi unknown admin route hit kare (e.g. /admin/xyz)
